...
```
                    онлайн                          оффлайн
                       │                               │
[Simulator] ──WebSocket──> [Ingestion Service]         │ нет связи
                │                   │                  │
                │            [Redis Streams]            │ буферизация
                │            (telemetry:raw)            │ со скоростью 1 Hz
                │                   │                  │ в JSONL-файл
                │                   v                  │ (кольцевой буфер,
                │          [Processing Service]         │  макс. 30 мин)
                │           /            \              │
                │    (напрямую)      (асинхронно)       │
                │          /              \             │
                │  [Redis Pub/Sub]    [TimescaleDB] <───┘ при восстановлении
                │  metrics:live           │              связи: replayed-данные
                │          │             │              попадают ТОЛЬКО сюда
                │          +──────┬──────+              (Pub/Sub пропускается)
                │                 │
...
### Offline-буферизация и воспроизведение

Когда симулятор теряет связь с Ingestion Service:

1. **Буферизация** — телеметрия продолжает генерироваться со скоростью 1 Hz и сохраняется в JSONL-файл на диске (`/tmp/loco_buffers/<train_id>.jsonl`). Кольцевой буфер ограничен 1800 записями (~30 минут).
2. **Восстановление** — при восстановлении связи live-поток возобновляется **немедленно**. Параллельно запускается фоновая задача, которая отправляет накопленные данные через отдельное WebSocket-соединение со скоростью 1 Hz.
3. **Только история** — воспроизведённые сообщения помечаются флагом `_replay: true`. Processing Service записывает их в TimescaleDB с исходными временными метками, но **не публикует** в Redis Pub/Sub — фронтенд не получает устаревшие данные.
...
| `OFFLINE_HZ` | `1` | Частота буферизации при отсутствии связи |
| `BUFFER_CAP` | `1800` | Максимум записей в буфере на локомотив (~30 мин при 1 Hz) |
| `BUFFER_DIR` | `/tmp/loco_buffers` | Директория для JSONL-буферов |
| `REPLAY_HZ` | `1` | Скорость воспроизведения буфера при восстановлении связи |
